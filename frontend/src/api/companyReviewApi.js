...
export async function fetchReviews(companyId) {
    const res = await api.get(`/companies/${companyId}/review`);
    return res.data;
}

// 기업 후기 상세 조회
export async function fetchReview(companyId, reviewId) {
    const res = await api.get(`/companies/${companyId}/review/${reviewId}`);
    return res.data;
}

// 기업 후기 작성
export async function createReview(companyId, payload) {
    const res = await api.post(`/companies/${companyId}/review`, payload);
    return res.data;
}
...
export async function updateReview(companyId, reviewId, payload) {
    const res = await api.put(
        `/companies/${companyId}/review/${reviewId}`,
        payload
    );
    return res.data;
}
...
export async function deleteReview(companyId, reviewId) {
    await api.delete(`/companies/${companyId}/review/${reviewId}`);
}